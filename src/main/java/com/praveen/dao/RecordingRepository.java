...
public interface RecordingRepository extends JpaRepository<Recordings, Integer> {	
	...
	@Query(value="select call_logs.assigned_to,call_logs.call_end_date,call_logs.call_date,call_logs.status,call_logs.call_duration,recordings.recording_name,leads.phone_number,leads.filename,recordings.campaing from recordings INNER JOIN call_logs ON call_logs.recording=recordings.recording_name INNER JOIN leads ON leads.id = recordings.lead_id where call_logs.assigned_to IN (:users) and call_logs.call_date >= :fromDate and call_logs.call_end_date <= :toDate or leads.phone_number=:phoneNumber  order by call_logs.call_date desc LIMIT :limit OFFSET :offset", nativeQuery = true)
