...
public interface CallLogsRepository extends JpaRepository<CallLogs, Integer> {
	@Query(value="select call_logs.assigned_to,table1.phone_number,call_logs.status,table1.first_name,call_logs.comments,call_logs.call_duration,call_logs.call_date,call_logs.call_end_date,recordings.recording_name from call_logs INNER JOIN leads as table1 ON table1.id = call_logs.lead_id LEFT JOIN recordings ON recordings.lead_id=call_logs.lead_id where  call_logs.call_date >= :fromDate and call_logs.call_end_date <= :toDate and call_logs.assigned_to IN (:userNames) ", nativeQuery = true)
	 ...
	 @Query(value="select call_logs.assigned_to,table1.phone_number,call_logs.status,table1.first_name,call_logs.comments,call_logs.call_duration,call_logs.call_date,call_logs.call_end_date,recordings.recording_name from call_logs INNER JOIN leads as table1 ON table1.id = call_logs.lead_id LEFT JOIN recordings ON recordings.lead_id=call_logs.lead_id where call_logs.call_date >= :fromDate and call_logs.call_end_date <= :toDate and table1.phone_number=:phoneNumber ", nativeQuery = true)
